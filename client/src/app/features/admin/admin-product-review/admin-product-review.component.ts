    ...
    if (productId) {
      ...
      this.productService.getProduct(parseInt(productId)).subscribe({
        next: (product: any) => {
          ...
          // Handle the image URL properly for display
          if (product.pictureUrl) {
            ...
            if (product.pictureUrl.startsWith('http://') || product.pictureUrl.startsWith('https://')) {
              ...
              // If it's a relative path, construct the full URL for client/public/images/products
              const clientBaseUrl = window.location.origin; // This will be http://localhost:4200 or your client URL
              // Ensure the path starts with /images/ if it doesn't already
              const imagePath = product.pictureUrl.startsWith('/images/') ? product.pictureUrl : `/images/${product.pictureUrl}`;
              this.product.pictureUrl = `${clientBaseUrl}${imagePath}`;
            }
          }
